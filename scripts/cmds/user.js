export default {
  ...
  onRun: async ({ event, args, message, setUserBanned }) => {
    ...
    if (!jid) {
      return message.reply("Please mention the user you want to ban or unban.");
    }

    const action = (args[1] || "").toLowerCase();
    if (action === "ban" || action === "-b") {
      await setUserBanned(jid, true);
      await message.reply(`@${jid.split("@")[0]} has been banned from using the bot.`, {
        mentions: [jid],
      });
      ...
      await message.reply("Usage: .user <@user> ban|-b or .user <@user> unban|-un");
    }
  },
};